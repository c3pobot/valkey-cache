...
module.exports.set = async({ allyCode, playerId, guildId, guildName })=>{
  try{
    ...
    return await cache.setJSON(`${KEY_PREFIX}:${guildId}`, { allyCode, playerId, guildId, guildName }, 604800);
  }catch(e){
    log.error(e)
  }
};
module.exports.get = async({ allyCode,  playerId })=>{
  try{
    if(!playerId && !allyCode) return;

    let tempId = playerId
    if(!tempId) tempId = await playerIdCache.get(allyCode);
    if(!tempId) return;
